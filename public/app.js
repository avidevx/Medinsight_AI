...
function main() {
    ...
    // Add click listeners to Resources and Support links
    const resourceLinks = document.querySelectorAll('.footer-column:nth-child(3) .footer-link');
    const supportLinks = document.querySelectorAll('.footer-column:nth-child(4) .footer-link');

    function addToastListeners(links) {
        links.forEach(link => {
            link.addEventListener('click', (e) => {
                e.preventDefault();

                // Handle Help Center specifically
                if (link.textContent.trim() === 'Help Center') {
                    const contactSection = document.querySelector('.footer-contact-info');
                    if (contactSection) {
                        contactSection.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        contactSection.classList.add('highlight-active');

                        // Remove class after animation completes
                        setTimeout(() => {
                            contactSection.classList.remove('highlight-active');
                        }, 2000);
                    }
                    return;
                }
    ...
    if (resourceLinks) addToastListeners(resourceLinks);
    if (supportLinks) addToastListeners(supportLinks);

    // Also add for social links if any
    const socialLinks = document.querySelectorAll('.social-link');
    if (socialLinks) addToastListeners(socialLinks);
